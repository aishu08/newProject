...
const occAgg = require('../models/occAggregation');
        ...
        else if(user)
        {
            bcrypt.compare(data.password, user.password, function(err, status){
                if(status)
                {
                    ...
                    res.status(200).json({name: data.uname, auth_token: myToken});
                }
                else
                {
    ...
    UserModel.findOne({name: data.uname}, function(err, user, num){
        ...
        else if(user)
        {
            ...
            bcrypt.compare(data.oldPassword, user.password, function(err, status){
                if(status)
                {
                    UserModel.findOneAndUpdate({name: data.uname},{password: bcrypt.hashSync(data.newPassword1)}, function(err, doc){
                        if(err)
                            res.status(400).json(err)
                        else{
        ...
        else
        {
            ...
            else
            {
                ...
                FloorModel.findById(floorId).lean().populate({path: 'sections', match:{name: data.bu}, select: " -isRoom -capacity"}).exec(function(err, sections){
                    ...
                    SeatToBle.find({seatId : {$in: seats}}, function(err, bles){
                        var bleIds = bles.map(function(ble){return {id: ble.bleId}})
                        if(bleIds.length > 0)
                        {
