app.controller("lightConfDwnldCtrl", ['$scope', '$rootScope', '$cookies',
    '$log',
    '$state',
    '$timeout',
    '$location',
    ...
    function($scope, $rootScope, $cookies, $log, $state, $timout, $location, $transitions, $http, $mdDialog) {
            ...
            if (!$scope.selectedhost || !$scope.selectedhost.id) {
                ...
                if (selectedobj.selected) {
                    ...
                    if (totaldevicecount > 150) {
                        ...
                        $http.put('/lightcommission/api/checkMappingwithotheHost', { ids: [selectedobj._id], hid: $scope.selectedhost.id }).then(function(response) {
        ...
        $scope.UpdateHost = function(hostobj, hostform) {
            ...
            if (hostform.$valid) {
                ...
                $http.post('/light/updateHost', hostobj).then(function(response) {
                    console.log("hostupdate", response);
                    $scope.closeDialog();
        ...
        $scope.MapAllLightsAndSensors = function() {
            ...
            $scope.makingAjaxCall = true;
            $http.put('/light/MapDevciesWithHost', { lights: finallightsforupdate, sensors: finallightsensorsforupdate, touchpanels: finaltouchpanlesforupdate, hid: $scope.selectedhost.id }).then(function(mapping) {
        ...
        $scope.SendLightsToHost = function() {
            ...
            $http.post('/light/publishHostDevices', { cloudConfig: finalbodytohost }).then(function(response) {
                $scope.makingAjaxCall = false;
                var data = response.data;
                $rootScope.showToast("Host data sent successfully", 'success-toast', 'top center')
        ...
        $scope.dwnldConfig = function() {

            $scope.makingAjaxCall = true;

            getmappedlightsandsensors(function() {
                console.log("i am inside getmapp")
                var finalbodytohost = {}
                ...
                if ($scope.selectedfloor._id) {
                    ...
                    finalbodytohost.tunableLightConfig = [];


                    for (var i = 0; i < $scope.lights.length; i++) {
                        if ($scope.lights[i].selected) {
                            if ($scope.lights[i].isTunable == false) {
                                var obj = {
                            ...
                            } else if ($scope.lights[i].isTunable == true) {
                                var obj = {
                                    "rotate": $scope.lights[i].rotate,
                                    "name": $scope.lights[i].name,
                                    "width": $scope.lights[i].width,
                                ...
                                finalbodytohost.tunableLightConfig.push(obj);
                            }
                    ...
                    console.log("final", finalbodytohost)
