app.controller('meetingRoomMgtCtrl', ['$rootScope','$scope','$mdDateRangePicker','$http','$timeout','$mdDialog','$stateParams','$filter' , '$mdMenu', '$timeout' , '$interval' , function($rootScope,$scope, $mdDateRangePicker, $http,$timeout,$mdDialog, $stateParams, $filter, $mdMenu, $timeout, $interval) {
    $rootScope.userContent();

    // var buildingId = $stateParams.bldgId;
    // $scope.building = $stateParams.bldgId;
    $scope.pageTitle = "Meeting-Room-Management";
...
//*************************************Update_Layout********************************
    ...
    $scope.setfloor = function(building){
        $http.get('/app/api/floors/'+building.bldgId).then(function(response){
            ...
            $scope.pageTitle = "Meeting-Room-Management - " + $scope.selectedFloor.name
    ...
    $scope.setBus = function(floor){
        $http.get('/app/api/getSectionNames/'+floor._id).then(function(response){
            ...
            $scope.pageTitle = "Meeting-Room-Management - " + floor.name;
            $scope.rooms = [];
...
//*******************************Popup-Categories**************************************
    ...
    $scope.savePos = function(){
        ...
        $http.post('/app/api/saveRoomConfiguration', $scope.rooms).then(function(response){
            console.log(response);
            $rootScope.showToast(response.data.msg, 'success-toast', 'top center')
