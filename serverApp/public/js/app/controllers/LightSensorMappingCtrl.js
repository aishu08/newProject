app.controller("LightSensorMappingCtrl", ['$scope', '$rootScope', '$cookies',
    '$log',
    '$state',
    '$timeout',
    '$location',
    ...
    function ($scope, $rootScope, $cookies, $log, $state, $timout, $location, $transitions, $http, $mdDialog) {
        ...
        console.log('LightSensorMappingCtrl');
        ...
        $scope.openDialog = function ($event, item) {
            if ($scope.selectedfloor && $scope.selectedfloor._id) {
                ...
                if (item === "Lights") {
                    $scope.MapSelectedLightswithSensor();
                } else {
                    $scope.UpdateSensorPosition();
                }
            } else {
        ...
        $scope.items = [
            ...
            // { name: "Save Position", type: "LightPosition", direction: "top", icon: "event_seat" }
        ];
        ...
        $scope.toggleselection = function (event, light) {
            ...
            if (event.ctrlKey) {
                if (!$scope.selectedsensor || !$scope.selectedsensor._id) {
                    $rootScope.showToast("Please select Sensor", 'error-toast', 'top center')
                } else {
                    ...
                    if (light.selected) {
                        ...
                        if (maxlights <= 20) {
                            ...
                            $http.put('/lightcommission/api/checkLightSensorMapping', { lightsids: [light._id], sensorid: $scope.selectedsensor._id }).then(function (response) {
        ...
        $scope.selectionStop = function (selected) {
            if (!$scope.selectedsensor || !$scope.selectedsensor._id) {
                ...
                if (selected.length > 0) {
                    ...
                    var maxlights = finallightsforupdate.length;
                    if (maxlights >= 20) {
                        $rootScope.showToast("You can only Map 20 Lights for selected Sensor", 'error-toast', 'top center')
                        //  $scope.markSensorassignedlights;
                    } else {
                        $http.put('/lightcommission/api/checkLightSensorMapping', { lightsids: ids, sensorid: $scope.selectedsensor._id }).then(function (response) {

                            console.log(response);
                            if (response.data.status) {

                                var confirm = $mdDialog.confirm()
                                    .title('Some Lights are already mapped to other Sensor?')
                                    .textContent('Please unmap from other Sensors to continue.')
                                    .ariaLabel('confirmation')
                                    .targetEvent()
                                    .ok('OK');
        ...
        $scope.addsensors = function () {
            ...
            if ($scope.selectedfloor && $scope.selectedfloor._id) {
                ...
                $mdDialog.show({
                    templateUrl: 'template/addsensors',
        ...
        $scope.creatsensors = function (sensors, form) {
            var sensorstoadd = [];
            var type = sensors.type;
            var nos = 0;
            if (form.$valid) {
                ...
                sensorstoadd.push(obj);
                if (nos <= 10) {
                    $scope.buttonclicked = true;
                    $http.post('/light/creatsensors/' + $scope.selectedfloor._id,
                        { buid: $scope.selectedBuilding.id, sensorcount: sensorstoadd,sensorType: type }).then(function (response) {
                            $scope.closeDialog();
                            $scope.GetSensors();
                    ...
                    $rootScope.showToast("You can add Maximum of 10 Sensors at a time", 'error-toast', 'top center')
                }
            } else {
        ...
        $scope.showPopover = function (ev, sensor) {
            $scope.sensorDetail = sensor
            console.log(sensor)
            if(sensor.isSOS){
                $scope.sensorDetail.type = "sos"
            }else if(sensor.isDLS){
                $scope.sensorDetail.type = "dls"
            }
            $mdDialog.show({
                templateUrl: 'template/editsensor',
        ...
        $scope.UpdateSensor = function () {
            ...
            $http.put('/light/updatesensor', $scope.sensorDetail).then(function (response) {
        ...
        $scope.deleteSensor = function () {
            ...
            $scope.makingAjaxCall = true;
            $http.put('/lightcommission/api/RemoveSensor', $scope.sensorDetail).then(function (response) {
                $scope.selectedsensor = {};
                $scope.GetSensors()
                $scope.closeDialog();
                $scope.markSensorassignedlights();
        ...
        $scope.MapSelectedLightswithSensor = function () {
            ...
            if ($scope.selectedsensor && $scope.selectedsensor._id) {
                ...
                $scope.makingAjaxCall = true;
                $http.put('/lightcommission/api/MapSelectedLightswithSensor',
                    { lights: finallightsforupdate, sensortobleid: $scope.selectedsensor.lightsensortobles._id }).then(function (response) {
                        console.log(response.data);
                        //  console.log($scope.selectedsensor);
                        angular.forEach($scope.sensors, function (item, index) {

                            if ($scope.selectedsensor._id === item._id) {
                                console.log($scope.selectedsensor)
                                $scope.selectedsensor.lightsensortobles.lights = response.data.sensor.lights;
                                item = $scope.selectedsensor;
                            }
                        });
        ...
        $scope.UpdateSensorPosition = function () {
            ...
            if ($scope.sensors.length > 0) {
                ...
                $scope.makingAjaxCall = true;
                $http.put('/lightcommission/api/SaveSensorsPosition', { sensors: $scope.sensors }).then(function (response) {
                    console.log(response.data);
                    $scope.GetSensors();
                    $scope.selectedsensor = {};
                    $scope.makingAjaxCall = false;
        ...
        $scope.markSensorassignedlights = function () {
            ...
            if ($scope.selectedsensor && $scope.selectedsensor._id) {
                for (var i = 0; i < $scope.selectedsensor.lightsensortobles.lights.length; i++) {
                    for (var j = 0; j < $scope.lights.length; j++) {

                        if ($scope.lights[j]._id == $scope.selectedsensor.lightsensortobles.lights[i]) {
            ...
            if ($scope.selectedfloor._id) {
                ...
                for (var i = 0; i < $scope.lights.length; i++) {
                    ...
                    var obj = {
                        ...
                        "ble": $scope.lights[i].bles.address,
                        "type": 8
                ...
                //   $scope.makingAjaxCall = false;
                $http.post('/lightcommission/api/SubmitLightsToHost', { cloudConfig: finalbodytohost }).then(function (response) {
