app.controller("LightBleMappingCtrl", ['$scope', '$rootScope', '$cookies','$log','$state','$timeout','$location',
    ...
    function ($scope, $rootScope, $cookies, $log, $state, $timout, $location, $transitions, $http, $mdDialog) {
        ...
        $scope.showPopover = function (ev, light) {
            ...
            $scope.selectedLightForBle = light;
            $scope.lightDetail = light
            console.log(light);
            console.log(light.bleAddress)
            if(light.bleAddress.length){
                light.bleAddress.forEach(function(ble){
                    var address = {};
                    $scope.data.addresses.push({"address":ble})
        ...
        function checkUniqueAddr(){
            $scope.data.addresses = _.uniq($scope.data.addresses, "address");
        }
    
       $scope.addRow = function(index){
        checkUniqueAddr();
         var address = {address:""};
            if($scope.data.addresses.length <= index+1){
                 $scope.data.addresses.splice(index+1,0,address);
             }
         };
       
       $scope.deleteRow = function($event,address){
       var index = $scope.data.addresses.indexOf(address);
         if($event.which == 1)
            $scope.data.addresses.splice(index,1);
         }
        $scope.UpdateLight = function (light) {
            console.log($scope.selectedLightForBle);
            $scope.makingAjaxCall = true;
            console.log($scope.data.addresses);
            checkUniqueAddr();
            data = {};
            $scope.selectedLightForBle.address = $scope.data.addresses;
            data.light = $scope.selectedLightForBle;
            data.address = $scope.data.addresses;
            $scope.makingAjaxCall = false;

            $http.put('/light/UpdateLightBle', data).then(function (response) {
                $scope.makingAjaxCall = false;
                console.log(response);
